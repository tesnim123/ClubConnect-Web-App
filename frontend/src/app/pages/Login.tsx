...
export default function Login() {
  ...
  const loggedInUserRole = useRef<string | null>(null); // Stocker le rôle du login

  // Redirection après authentification réussie
  useEffect(() => {
    if (isAuthenticated && !loading) {
      // Priorité au rôle stocké lors du login, sinon utiliser le user du contexte
      const role = loggedInUserRole.current || user?.role;
      
      if (role) {
        console.log("Redirecting with role:", role);
        const redirectTo = location.state?.from?.pathname || getRoleHomePath(role as any);
        console.log("Redirecting to:", redirectTo);
        navigate(redirectTo, { replace: true });
      }
    }
  }, [isAuthenticated, loading, user, navigate, location.state?.from?.pathname]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // Stocker le rôle immédiatement
      loggedInUserRole.current = loggedInUser.role;
      
      toast.success("Connexion réussie");
      
      // Rediriger immédiatement avec le rôle du login
      const redirectTo = location.state?.from?.pathname || getRoleHomePath(loggedInUser.role);
      console.log("Immediate redirect to:", redirectTo);
      navigate(redirectTo, { replace: true });
        ...
        <section className="hidden lg:block">
          <div className="max-w-xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/60 bg-white/70 px-4 py-2 text-sm text-[#1B2A4A] shadow-sm backdrop-blur">
              ...
              Espace clubs, événements et animation jeunesse
            </div>
            ...
            <p className="mt-5 max-w-lg text-lg leading-8 text-slate-600">
              ClubConnect centralise les clubs, les demandes d'événements, les communications et les profils d'équipe dans un espace moderne et rapide.
            </p>
            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              {[
                { icon: ShieldCheck, title: "Accès sécurisé", text: "Connexion admin et rôles reliés au vrai compte." },
                { icon: LockKeyhole, title: "Session fiable", text: "Navigation, profil et déconnexion synchronisés." },
                { icon: Sparkles, title: "Expérience moderne", text: "Design plus adapté aux clubs et aux jeunes." },
        ...
        <Card className="mx-auto w-full max-w-lg rounded-[2rem] border-white/70 bg-white/90 p-8 shadow-[0_25px_80px_rgba(15,23,42,0.12)] backdrop-blur">
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">
              Accédez à votre espace administrateur, président, staff ou membre.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="rounded-2xl border border-[#0EA8A8]/15 bg-[#0EA8A8]/5 p-4 text-sm text-slate-600">
              L'accès administrateur utilise maintenant la même session fiable que le reste de l'application.
            </div>
